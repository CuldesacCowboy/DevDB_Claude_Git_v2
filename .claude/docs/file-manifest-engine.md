...
### devdb_python/engine/marks_builder_sync.py
- Owns: applies MARKS builder_id and is_spec to real/pre lots via devdb_ext.housemaster + codetail joins; uses LPAD zero-padded housenumber comparison (marks_mirror varchar format)
...
- Owns: Shared seasonal weight sets (month->fractional weight, sums to 1.0) used by demand_generator and placeholder_rebuilder for monthly demand/pace allocation
- Imported by: demand_generator.py, placeholder_rebuilder.py
- Tables: none
- Last commit: 2026-04-03

### devdb_python/engine/hc_bldr_date_projector.py
- Owns: writes BLDR/STR/CMP/CLS projected dates for HC-held lots based on demand allocation order; identifies HC lots (date_td_hold or date_td_hold_projected set, no date_td, no date_str, not locked); calls demand_allocator over full snapshot; BLDR = first-of-demand-month - td_to_str_lag, clamped to MAX(hold_floor) across building-group mates; STR = BLDR + lag; CMP/CLS derived via empirical lag curves; caches (lag_str_cmp, lag_cmp_cls) per building_group_id so all mates in a building share identical CMP/CLS dates; clears date_td_hold_projected when demand path covers lot before hold date; never writes actuals; falls back to pace-based demand when demand_series is empty
- Imports: engine.demand_allocator, engine.timing_expansion, pandas, datetime, random
- Imported by: coordinator.py
- Tables: sim_lots (UPDATE date_td_projected, date_str_projected, date_cmp_projected, date_cls_projected, date_td_hold_projected)
- Last commit: 2026-04-24

### devdb_python/engine/demand_generator.py
- Owns: generates monthly demand series for each phase; vectorized; capacity-capped per D-138
- Imported by: coordinator.py
- Tables: sim_dev_phases, sim_phase_product_splits, sim_lots (SELECT)
- Last commit: 2026-04-24

### devdb_python/engine/building_group_sync.py
- Owns: propagates MIN(date_str, date_cmp, date_td) within building groups for real/pre lots; runs between date_actualizer and gap_fill_engine; returns early if no lots have building_group_id set
- Imported by: coordinator.py
- Tables: none (pure DataFrame transform)
- Last commit: 2026-04-24

### devdb_python/engine/demand_allocator.py
- Owns: allocates demand slots to real/sim lots; building-group-aware: groups are atomic units (all lots in a group consume N demand slots and share the first slot's month); positional merge; no carry-forward; empty lot_snapshot does NOT short-circuit
- Imported by: kernel/planning_kernel.py
- Tables: none (pure DataFrame transform)
- Last commit: 2026-04-24

### devdb_python/engine/temp_lot_generator.py
- Owns: generates sim lots for unmet demand; date_str = demand slot month (or deferred); date_td = date_str per D-137/D-142; deferred-start logic; building group generation: when phase_building_config is provided, generates lots in groups using bg_templates; synthetic negative building_group_id values assigned (no DB FK); all units in a building share first slot's date_str
- Imported by: kernel/planning_kernel.py
- Tables: none (builds list; persistence is in persistence_writer)
- Last commit: 2026-04-24

### devdb_python/engine/building_group_enforcer.py
- Owns: enforces MIN(date_str) per building_group_id across sim lots per D-133
- Imported by: kernel/planning_kernel.py
- Tables: none (pure DataFrame transform)
- Last commit: 2026-04-24

### devdb_python/engine/post_gen_chronology_guard.py
- Owns: discards sim lots with chronology violations post-generation; warns on fully-cleared phases
- Imported by: kernel/planning_kernel.py
- Tables: none (pure DataFrame filter)
- Last commit: 2026-04-24

### devdb_python/engine/builder_assignment.py
- Owns: builder_assignment() assigns builder_id to sim/temp lots in memory (pure, no DB); uses _apply_splits_to_indices() helper; input builder_splits dict is {phase_id: [...]} loaded by coordinator via sim_instrument_builder_splits join
- Imported by: coordinator.py
- Tables: none (pure DataFrame transform)
- Last commit: 2026-04-24

### devdb_python/engine/real_lot_builder_assign.py
- Owns: assign_real_lot_builders() DB pre-pass assigns builder_id to real/pre lots where COALESCE(builder_id_override, builder_id) IS NULL using same proportional split logic as builder_assignment; called once per coordinator run before iteration loop (idempotent); shares _apply_splits_to_indices() helper
- Imported by: coordinator.py
- Tables: sim_lots (SELECT real/pre lots, UPDATE builder_id via execute_values)
- Last commit: 2026-04-24

### devdb_python/engine/demand_derived_date_writer.py
- Owns: writes MIN(date_str) per phase to sim_dev_phases.date_dev_projected
- Imported by: coordinator.py
- Tables: sim_dev_phases (UPDATE date_dev_projected)
- Last commit: 2026-04-24

### devdb_python/engine/d_bldr_date_projector.py
- Owns: writes BLDR/STR/CMP/CLS projected dates for D-status real lots (date_dev set, no hold, no date_td, no date_str, not locked); calls demand_allocator over full snapshot; BLDR = first-of-demand-month - td_to_str_lag; STR = demand_month_first; CMP/CLS via empirical lag curves; caches (lag_str_cmp, lag_cmp_cls) per building_group_id so all mates share identical CMP/CLS; falls back to pace-based demand when demand_series empty; DB write includes all four columns with AND date_td IS NULL + AND date_td_is_locked IS NOT TRUE guards; never writes actuals
- Imports: engine.demand_allocator, engine.timing_expansion, pandas, datetime, random, math
- Imported by: coordinator.py
- Tables: sim_lots (UPDATE date_td_projected, date_str_projected, date_cmp_projected, date_cls_projected)
- Last commit: 2026-04-24

### devdb_python/engine/timing_expansion.py
- Owns: derives date_cmp and date_cls from date_str for sim lots using empirical lag curves; pre-loop caches one (lag_str_cmp, lag_cmp_cls) pair per building_group_id using first unit's lot_type_id (both CMP and CLS cached per group -- all mates share identical dates per D-022); ungrouped lots sample independently; falls back to _default_cmp/_default_cls keys when no curve matches; also exports load_build_lag_curves(), curves_for(), sample_lag() helpers used by hc_bldr_date_projector, d_bldr_date_projector, real_lot_projections
- Imports: engine.connection, pandas, logging
- Imported by: coordinator.py (via load_build_lag_curves), hc_bldr_date_projector.py, d_bldr_date_projector.py, real_lot_projections.py
- Tables: sim_build_lag_curves (SELECT via load_build_lag_curves)
- Last commit: 2026-04-24

### devdb_python/engine/real_lot_projections.py
- Owns: writes projected STR/CMP/CLS dates to real P lots (excludes D-status lots -- those are handled by d_bldr_date_projector) at annual_starts_target pace from sim_dev_params; pre-clear excludes D-status lots (AND date_td_projected IS NULL guard); pace block runs only when P lots exist; HC group-mate sync runs unconditionally at end (not gated on pace block) -- propagates BLDR/STR/CMP/CLS from HC-assigned real lots to non-HC mates in same building group, overriding any d_bldr_date_projector value; skips locked lots; independent of kernel capacity logic
- Imported by: coordinator.py
- Tables: sim_lots (UPDATE date_td_projected, date_str_projected, date_cmp_projected, date_cls_projected), sim_dev_params (SELECT)
- Last commit: 2026-04-24
...
### devdb_python/engine/placeholder_rebuilder.py (P-0000)
- Owns: rebuilds placeholder delivery events per D-139 cross-dev scheduling lean rule; D-balance floor enforcement using min_d_count/per-status floors from sim_entitlement_delivery_config; uses delivery_months integer[] (frozenset) for window logic -- supports arbitrary month sets; Step 7 auto-generates sim_delivery_event_predecessors rows between consecutive events per dev (ordered by sequence_number) so dependency_resolver/delivery_date_assigner enforce absolute phase ordering; step 3c uses `date_ent IS NOT NULL` (not `date_dev IS NULL`) to detect real pending lots -- survives lot_date_propagator multi-iteration write-back; pd.NaT normalized to None at load time; delivery_tier gate in outer loop (tier-N held until all tier-(N-1) scheduled); dev_phases sorted by (tier, seq); tier check in inner co-bundling loop; feed_starts_mode=True bypasses both tier gates while predecessor links still enforce tier ordering
- Imported by: coordinator.py
...
### devdb_python/engine/actual_date_applicator.py
- Owns: applies locked delivery event dates to sim_dev_phases.date_dev_projected per D-112/D-125
- Imported by: coordinator.py
