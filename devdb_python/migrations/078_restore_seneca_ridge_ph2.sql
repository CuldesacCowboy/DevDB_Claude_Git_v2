...
-- Step 2-4 done via DO block so we can use the new phase_id.
DO $$
DECLARE
    new_phase_id INT;
BEGIN
    -- Step 2: Re-insert ph. 2 if it doesn't already exist.
    IF NOT EXISTS (
        SELECT 1 FROM sim_dev_phases
    ...
    -- Step 3: Product split (idempotent).
