...
def get_marks_summary(conn=Depends(get_db_conn)):
    ...
    try:
        cur.execute(f"""
            ...
            SELECT
                mlr.developmentcode,
                dm.description        AS marks_dev_name,
            ...
            GROUP BY mlr.developmentcode, dm.description, d.dev_name, d.dev_id
        ...
        return [
            {
                ...
                "dev_name":       r["app_dev_name"] or r["marks_dev_name"],
    ...
    try:
        cur.execute(f"""
            ...
            LEFT JOIN pivoted p
             ...
             AND p.housenumber     = LPAD(mlr.housenumber::text, 8, '0')
