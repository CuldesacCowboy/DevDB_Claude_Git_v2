            ...
            {sortedFiltered.map((l, idx) => {
              ...
              <tr key={l.lot_id} style={{ background: rowTint ?? '' }}>
                ...
                <td style={tdS('center', { width: 20 })}>
                  {l.is_spec === true && <span style={{ ...provStyle(l.is_spec_source === 'marks' ? 'marks' : l.is_spec_source === 'manual' ? 'override' : 'sim'), fontSize: 10, padding: '0 4px' }}>S</span>}
                </td>
                <td style={tdS('center', { width: 20 })}>
                  {l.is_spec === false && <span style={{ ...provStyle(l.is_spec_source === 'marks' ? 'marks' : l.is_spec_source === 'manual' ? 'override' : 'sim'), fontSize: 10, padding: '0 4px' }}>B</span>}
                </td>
