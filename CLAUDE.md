...
*Last updated: April 2026 (2026-04-23) | Architecture v20 | Decision Log: D-001 through D-167 | Next ID: D-168 | Migrations: 001–083*
...
| HC scheduling horizon | Complete | scheduling_horizon_days flows through build_lag_curves to S-0500; past checkpoints skip HC assignment (gap recorded only); S-0760 clears date_td_hold_projected when demand path covers lot before hold date |
...
| Engine modules | Complete | S-0100 through S-0900 PASS. S-1000 through S-1200 PASS. P-01 through P-08 PASS. Convergence coordinator PASS. S-0050 implemented 2026-04-15: reads devdb_ext.housemaster, applies MARKS builder_id to real/pre lots before S-0900 split assignment. S-0810 and S-0820 implemented 2026-03-25. P-0000 fix 2026-04-14: phases with real entitled lots but no demand signal (available_capacity=0) now schedule correctly; pd.NaT normalized to None; `date_ent IS NOT NULL` check survives P-07 multi-iteration write-back. P-0000 tier fixes 2026-04-15: (1) tier gate in active dict; (2) sort dev_phases by (tier, seq); (3) tier check in inner co-bundling loop; (4) feed_starts_mode bypass for aggressive batching. S-0700 fix: empty lot_snapshot no longer short-circuits unmet demand. S-0800 fix: deferred-start logic — lots pre-delivery date spread at 1/month after delivery. frozen_input_builder: phase capacity sorted by (tier, seq, phase_id). Building group enforcement (2026-04-14): S-0205 (real lot group sync), S-0700 building-group-aware allocation, S-0800 grouped sim lot generation with phase_building_config, S-0810 enforcer, FrozenInput.phase_building_config field, migration 056 (sim_phase_building_config table). All 14/14 Pokemon tests and 6/6 kernel scenarios pass. TDA fixes 2026-04-18/19: S-0500 NULL guards for checkpoint_number and lots_required_cumulative (crash fixes); S-0500 DB+in-memory pre-clear extended to also wipe stale date_td_projected written by S-0760 so _available() sees clean lots each iteration; S-0760 hc_bldr_date_projector new stage — after S-06, runs demand allocator, writes date_td_projected for HC-held lots (first of allocated demand month); coordinator wired S-0760 between S-06 and kernel pass. Building-group date invariant (2026-04-21): S-0500 syncs MAX(date_td_hold_projected) to all group mates post-assign + extended pre-clear for group mates; S-0760 caches (lag_str_cmp, lag_cmp_cls) per building_group_id so all HC mates share identical CMP/CLS; S-0770 extended to write all four projected dates (BLDR/STR/CMP/CLS) not just BLDR, also caches lag per group; S-0850 adds bg_cls_lag cache alongside existing bg_cmp_lag so sim lots share CLS; S-1050 HC group-mate sync runs unconditionally (not gated on pace block) and now syncs BLDR too; S-1050 excludes D-status lots from pace model and pre-clear. |
...
| Rules Validator | Complete | SimulationView "Rules Validator" tab. 20 rules in 3 categories: Config Completeness (product splits, starts target, builder splits, delivery config); Delivery Rules (window, max/yr, tier ordering with flow diagram, group simultaneous with group cards, group exclusivity, sequence ordering, all scheduled, locked honored); Engine Diagnostics (chronology, builder coverage, spec/build, building group sync, TDA fulfillment with checkpoint table, demand/capacity, convergence, pipeline monotonicity). Expandable rows with rich detail. |
