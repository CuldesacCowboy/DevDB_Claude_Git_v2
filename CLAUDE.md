...
*Last updated: April 2026 (2026-04-30) | Architecture v20 | Decision Log: D-001 through D-168 | Next ID: D-169 | Migrations: 001–091*
<!-- session 2026-04-27/28/29/30: HC demand double-counting fix (demand_allocator + coordinator _build_sim_demand); read-only scenario runner (Phases 1-6); ephemeral projections (Phases 1-8: sim_lots purged, sim_projection_lots + sim_projections tables, v_sim_ledger_combined view, persistence_writer sole target, supply pipeline redirected); scenario convergence fix (full loop instead of simplified pipeline); MARKS import fixes (housenumber varchar cast, devmaster FDW for dev names); unified setup page (merged ConfigView into SetupView with mode bar); scenario comparison aggregation fix (per-dev → per-month); utilization moved to delivery schedule column; Ravines at Inwood South phase reassignment -->
...
| Delivery groups | Complete | Migration 081: delivery_group CHAR(1) A-Z on sim_dev_phases. placeholder_rebuilder: group enforcement pulls same-group phases into same event; group exclusivity rule blocks non-group phases from group dates; locked group dates pre-blocked. locked_event_rebuilder: pulls unlocked group members into locked events. Admin API + PhaseTab + DeliveryScheduleTab all support delivery_group. |
...
| Rules Validator | Complete | SimulationView "Rules Validator" tab. 22 rules in 3 categories: Config Completeness (product splits, starts target, builder splits, delivery config, ledger dates); Delivery Rules (window, delivery-after-entitlement, max/yr, tier ordering with grid timeline, group simultaneous with group cards, group exclusivity, sequence ordering with instrument grid, all scheduled, locked honored); Engine Diagnostics (chronology, builder coverage, spec/build, building group sync, TDA fulfillment with checkpoint table, demand/capacity, convergence, pipeline monotonicity). Article-style expandable detail per rule: explanation, methodology, full data tables, visual diagrams (tier grid with staircase layout + common prefix stripping, sequence grid, pipeline diagram, group cards), written conclusions. Failed rules show fix directions + navigation buttons to correct page/tab. auto_schedule_enabled stripped (migration 084). temp_lot_generator date_td clamped >= date_dev. placeholder_rebuilder floors delivery dates to entitlement date. tda_preclear HC under-assignment fix: projected_natural now counts lots with date_td_projected <= checkpoint instead of pace estimate. |
| Scenario comparison | Complete | Migration 086: sim_scenarios, sim_scenario_overrides, sim_scenario_results. CRUD API for scenarios with parameter overrides (annual_starts_target, max_deliveries_per_year, delivery_months, spec_rate). Read-only scenario runner: engine accepts overrides in-memory, writes to sim_projection_lots (scenario projection) + sim_scenario_results. Never touches sim_lots. Compare endpoint returns base + scenario results. ScenarioPanel tab in SimulationView. LedgerGraph overlay: dashed lines for scenario projections (purple/pink/orange). Purple comparison banner with Clear button. |
| Ephemeral projections | Complete | Migrations 090-091. sim_lots holds ONLY real/pre lots (CHECK constraint enforced). Sim lots live in sim_projection_lots tagged by projection_id. sim_projections table tracks base and scenario projections (one current base per community). v_sim_ledger_combined view UNIONs real lots from sim_lots + sim lots from current base projection. v_sim_ledger_monthly reads from combined view. All API endpoints (ledger, utilization, rules validation, portfolio, admin) redirected to combined view. Supply pipeline (delivery_scheduler, delivery_phase_collector) reads/writes sim lots via projection_id. persistence_writer writes exclusively to sim_projection_lots. HC demand double-counting fix: hc_bldr_date_projector runs before kernel, sim demand shifted to earliest achievable supply, HC starts deducted at month level. |
| Unified setup page | Complete | ConfigView merged into SetupView. Mode bar: Structure (tree) / Communities / Developments / Instruments / Phases (spreadsheets). Config tabs lazy-load on first switch. /configure route redirects to /setup. AuditView + SimulationView deep-links updated. ConfigView.jsx removed from routing. Mode persisted to localStorage. |
| MARKS import fixes | Complete | devdb_ext.devmaster FDW table (company 001 preferred, fallback any). marks.py unimported + import queries fixed for varchar housenumber (LPAD cast). MarksView shows MARKS Name (devmaster.description) + App Development (developments.dev_name) columns. |
| Portfolio dashboard | Complete | New /portfolio route. GET /portfolio/summary endpoint aggregates v_sim_ledger_monthly across communities via sim_ent_group_developments join. Status filter pills (Active/Prospective/Sold Out/etc). 4 summary cards (communities, starts YTD, unstarted inventory, total projected). Production Curve chart (starts/completions/closings). Inventory Pipeline chart (D/H/U/UC/C stacked areas). Builder Capacity table with sortable columns and utilization bars. |
| Decision log | Current | D-168 added. Next ID: D-169. |
...
| Builder data | Complete | Three-tier priority: builder_id_override (user manual) > builder_id (MARKS via housemaster.csv import) > NULL (builder_assignment assigns via splits). Migration 040: builder_id_override on sim_lots. Migration 041: marks_company_code on dim_builders. real_lot_builder_assign assigns builder_id to real/pre lots with COALESCE(builder_id_override, builder_id) IS NULL before iteration loop. |
...
| Engine Reference | `.claude/docs/engine-reference.md` | Simulation engine, coordinator, delivery scheduling, placeholder_rebuilder/demand_generator, ledger, Databricks context |
